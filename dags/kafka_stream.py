...
default_args = {"owner": "airflow", "start_date": datetime(2024, 4, 19, 00)}
...
def getTrafficCrashData():
    ...
    res = res.json()
    log.info(
        "getTrafficCrashData > Done parsing result from API call.. returning result to caller"
    )
    return res
...
def getTrafficCongestionData():
    ...
    congestion_raw_data = requests.get(
...
def publishFormattedTrafficCrashData(res: List[Dict[str, Any]]):
    ...
    try:
        for idx, record in enumerate(res):
            ...
            log.info(
                "%s publishFormattedTrafficCrashData > data formatted, sending data to Kafka",
                idx,
            )
            producer.send("dummy", formatted_record)
            log.info(
                "%s publishFormattedTrafficCrashData > data sent to Kafka, continuing..",
                idx,
            )
...
with DAG(
    ...
    streaming_task = PythonOperator(
        task_id="stream_data_from_api_to_kafka", python_callable=stream_data
    )
